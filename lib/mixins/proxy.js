 ...
 */
function requireAndroidDriver(driver) {
  if (!driver) {
    throw new Error(
      `The current driver does not have the required 'adb' property. ` +
...
function requireDriverProperties(driver) {
  for (const propName of ['serverInfo', 'sessionId']) {
    if (!driver?.[propName]) {
      throw new Error(
...
export async function listDevtoolsTargets () {
  requireAndroidDriver(this.driver);
...
export async function proxyDevtoolsTarget (name, port) {
  ...
  const alias = toSocketNameAlias(name);
  /** @type {import('../plugin').ServerInfo} */
  // @ts-ignore Nullability is verified by requireDriverProperties
  ...
  const serverPath = serverInfo.path.replace(/\/$/, '');
  const forwardTo =
  ...
  rewrites.push([
    // replace params
    `ws=${wdUrl.host}`,
    `ws=${serverHost}:${serverPort}${serverPath}/session/${this.sessionId}/${CDP_METHODS_ROOT}/${alias}`,
  ], [
    `"/devtools/`,
