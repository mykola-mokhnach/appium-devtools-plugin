...
function requireAndroidDriver(driver) {
  if (!driver?.adb) {
    throw new Error(
      `The ${driver?.constructor?.name} does not have the required 'adb' property. ` +
...
export async function proxyDevtoolsTarget (name, port) {
  ...
  const prepareWebSocketForwarder = (from, key) => {
    ...
    wss.on('connection', (ws, /** @type {import('http').IncomingMessage} */ req) => {
      ...
      const dstUrl = from.replace(new RegExp(`:${key}`, 'g'), req.params[key]);
      this.logger.debug(`Will forward to ${dstUrl}`);
      ...
      wsClient.once('error', (e) => {
        this.logger.warn(`Got an error from the source socket: ${e.message}`);
      ...
      wsClient.once('close', (code, reason) => {
        this.logger.info(`The source socket has been closed: ${code}, ${reason}`);
  ...
  this.logger.debug(
    `Successfully started the proxy for the Devtools target '${name}': ` +
