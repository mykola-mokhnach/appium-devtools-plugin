...
function extractWsEntityId (pathname) {
  const browserMatch = /\/devtools\/browser\/([a-fA-F0-9-]+)/.exec(pathname);
  if (browserMatch) {
    return browserMatch[1];
  }
  const pageMatch = /\/devtools\/page\/([a-fA-F0-9-]+)/.exec(pathname);
  if (pageMatch) {
    return pageMatch[1];
  }
  return null;
...
export async function proxyDevtoolsTarget (name, port) {
  ...
  const prepareWebSocketForwarder = (from, entityIdPlaceholder) => {
    ...
    wss.on('connection', (ws, /** @type {import('http').IncomingMessage} */ req) => {
      ...
      const dstUrl = from.replace(entityIdPlaceholder, entityId);
  ...
  const browserDebuggerUrlPattern = `${new URL(forwardTo).pathname}/devtools/browser/${browserIdPlaceholder}`;
  await server.addWebSocketHandler(
    browserDebuggerUrlPattern,
    prepareWebSocketForwarder(
      browserDebuggerUrl.replace(
        _.last(browserDebuggerUrl.split('/')), browserIdPlaceholder
      ),
  ...
  const pageDebuggerUrlPattern = `${new URL(forwardTo).pathname}/devtools/page/${pageIdPlaceholder}`;
  await server.addWebSocketHandler(
    pageDebuggerUrlPattern,
    prepareWebSocketForwarder(
      browserDebuggerUrl.replace('/browser/', '/page/').replace(
        _.last(pageDebuggerUrlPattern.split('/')), pageIdPlaceholder
      ),
      pageIdPlaceholder
    )
  );
