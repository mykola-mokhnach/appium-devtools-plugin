...
function requireDriverProperties(driver) {
  const result = {};
  for (const propName of ['adb', 'server', 'sessionId']) {
    if (!driver?.[propName]) {
      throw new Error(
 ...
 */
async function getCandidateSocketNames(adb) {
 ...
 */
async function collectMultipleDetails(adb, socketNames) {
 ...
 */
export async function listDevtoolsTargets (next, driver) {
  const {adb} = requireDriverProperties(driver);

  const socketNames = await getCandidateSocketNames.bind(this)(adb);
  const webviewsMapping = await collectMultipleDetails.bind(this)(adb, socketNames);
...
export async function proxyDevtoolsTarget (next, driver, name, port) {
  const {adb, server, sessionId} = requireDriverProperties(driver);
  ...
  const prepareWebSocketForwarder = (forwardToUrlPattern, entityIdPlaceholder) => {
    ...
    wss.on('connection', (ws, /** @type {import('http').IncomingMessage} */ req) => {
      this.logger.debug(`Got a new websocket connection from ${req.url}`);
      if (!req.url) {
      ...
      if (!entityId) {
        this.logger.debug(`Cannot parse the entity identifier from '${pathname}'. Will ignore`);
        return;
      }
      const dstUrl = forwardToUrlPattern.replace(entityIdPlaceholder, entityId);
      this.logger.debug(`Will forward ${req.url} to ${dstUrl}`);
      ...
      wsClient.once('error', (e) => {
        this.logger.warn(`Got an error from the source socket ${dstUrl}: ${e.message}`);
        ws.close(WS_SERVER_ERROR, e.message);
      });
      ws.once('error', (e) => {
        this.logger.info(`Got an error from the client socket: ${e.message}`);
        wsClient.close(WS_SERVER_ERROR, e.message);
      });
      wsClient.once('close', (code, reason) => {
        this.logger.info(`The source socket ${dstUrl} has been closed: ${code}, ${reason}`);
        ws.close(code, reason);
      });
      ws.once('close', (code, reason) => {
        this.logger.info(`The client socket has been closed: ${code}, ${reason}`);
  ...
  const {address: serverHost, port: serverPort} = server.address();
  const serverPath = (driver.serverPath ?? '/').replace(/\/$/, '');
  const forwardTo =
    `${serverProtocol}://${serverHost}:${serverPort}${serverPath}/session/${sessionId}/${CDP_METHODS_ROOT}/${alias}`;
  /** @type {URL} */
  let wdUrl;
  try {
  ...
  rewrites.push([
    // replace params
    `ws=${wdUrl.host}`,
    `ws=${serverHost}:${serverPort}${serverPath}/session/${sessionId}/${CDP_METHODS_ROOT}/${alias}`,
  ], [
    `"/devtools/`,
    `"${serverPath}/session/${sessionId}/${CDP_METHODS_ROOT}/${alias}/devtools/`
  ]);

  const entityId = extractWsEntityId(browserDebuggerUrl);
  if (!entityId) {
    await removePortForward();
    throw new Error(
      `The target '${name}' cannot be proxied. ` +
      `The '${DEBUGGER_URL_KEY}' key value '${browserDebuggerUrl}' in the /json/version response is not a valid browser debugger URL`
    );
  }

  const browserIdPlaceholder = ':browserId';
  const browserDebuggerPathname = `${new URL(forwardTo).pathname}/devtools/browser/${browserIdPlaceholder}`;
  const pageIdPlaceholder = ':pageId';
  const pageDebuggerPathname = `${new URL(forwardTo).pathname}/devtools/page/${pageIdPlaceholder}`;

  for (const [placeholder, fromPathname, toUrlPattern] of [[
    browserIdPlaceholder,
    browserDebuggerPathname,
    browserDebuggerUrl.replace(entityId, browserIdPlaceholder),
  ], [
    pageIdPlaceholder,
    pageDebuggerPathname,
    browserDebuggerUrl.replace('/browser/', '/page/').replace(entityId, pageIdPlaceholder),
  ]]) {
...
export async function unproxyDevtoolsTarget (next, driver, name) {
  const {adb, server} = requireDriverProperties(driver);
