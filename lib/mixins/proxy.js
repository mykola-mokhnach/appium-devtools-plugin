...
export async function proxyDevtoolsTarget (name, port) {
  ...
  const prepareWebSocketForwarder = (from, key) => {
    ...
    wss.on('connection', (ws, /** @type {import('http').IncomingMessage} */ req) => {
      this.log.debug(`Got a new websocket connection from ${req.url}`);
      const dstUrl = from.replace(new RegExp(`:${key}`, 'g'), req.params[key]);
      this.log.debug(`Will forward to ${dstUrl}`);
      ...
      wsClient.once('error', (e) => {
        this.log.warn(`Got an error from the source socket: ${e.message}`);
        ws.close(WS_SERVER_ERROR, e.message);
      });
      ws.once('error', (e) => {
        this.log.info(`Got an error from the client socket: ${e.message}`);
        wsClient.close(WS_SERVER_ERROR, e.message);
      });
      wsClient.once('close', (code, reason) => {
        this.log.info(`The source socket has been closed: ${code}, ${reason}`);
        ws.close(code, reason);
      });
      ws.once('close', (code, reason) => {
        this.log.info(`The client socket has been closed: ${code}, ${reason}`);
